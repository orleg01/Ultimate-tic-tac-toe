...
namespace WpfApplication1.GameLogic
{
    class LittleBoard
    {
        ...
        public int LenOfComputerPositionForDebug
        {
            get
            {
                int num = 0;
                for (int i = 0; i < 3; i++)
                    for (int j = 0; j < 3; j++)
                        if (board[i, j] == PositionOnBoard.COMPUTER)
                            num++;
                return num;
            }
        }
        public int LenOfPlayerPositionForDebug
        {
            get
            {
                int num = 0;
                for (int i = 0; i < 3; i++)
                    for (int j = 0; j < 3; j++)
                        if (board[i, j] == PositionOnBoard.PLAYER)
                            num++;
                return num;
            }
        }
